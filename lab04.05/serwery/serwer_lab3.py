...
def _handle_client(connection: socket.socket, address: tuple[str, int]) -> None:
	...
	print(f"[serwer_lab3] Client connected: {address[0]}:{address[1]}")
	print(f"[serwer_lab3] C: {request_line}")
	...
	if range_header:
		...
		connection.sendall(response)
		print("[serwer_lab3] Session finished (range)")
		return
...
def main() -> None:
	print(f"[serwer_lab3] HTTP server listening on {HOST}:{PORT}")
