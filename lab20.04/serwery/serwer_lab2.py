...
def handle_client(conn: socket.socket, addr: tuple[str, int]) -> None:
    print(f"[serwer_lab2] Klient polaczony: {addr[0]}:{addr[1]}")
    ...
    send_line(conn, "+OK POP3 lab2 ready")
    ...
    while True:
        ...
        if command == "USER":
            ...
            if not authed:
                ...
                send_line(conn, f"+OK {len(MESSAGES)} messages ({total_octets} octets)")
                for idx, size in enumerate(message_sizes, start=1):
                    send_line(conn, f"{idx} {size}")
                send_line(conn, ".")
        elif command == "QUIT":
    ...
    print(f"[serwer_lab2] Klient rozlaczony: {addr[0]}:{addr[1]}")


def main() -> None:
    print(f"[serwer_lab2] Start on {HOST}:{PORT}")
    print(f"[serwer_lab2] Login: {USERNAME}")
    print(f"[serwer_lab2] Haslo: {PASSWORD}")
