...
def run_server() -> None:
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
		...
		print(f"[serwer_lab7] SMTP text attachment server on {HOST}:{PORT}")
		print("[serwer_lab7] Waiting for one client...")

		connection, address = server_socket.accept()
		with connection:
			print(f"[serwer_lab7] Client connected: {address}")
			...
			while True:
				...
				if not chunk:
					print("[serwer_lab7] Client disconnected")
					break
				...
				while b"\n" in buffer:
					...
					if data_mode:
						if line == ".":
							...
							has_text_attachment = "content-type: text/plain; name=" in joined

							print(f"[serwer_lab7] MIME multipart present: {has_multipart}")
							print(f"[serwer_lab7] Base64 attachment marker present: {has_base64}")
							print(f"[serwer_lab7] Text attachment marker present: {has_text_attachment}")

							if has_multipart and has_base64 and has_text_attachment:
								send_line(connection, "250 MIME text attachment message accepted")
							else:
								send_line(connection, "550 MIME text attachment headers missing for zad7")
					...
					if upper_line.startswith("EHLO") or upper_line.startswith("HELO"):
						...
						print("[serwer_lab7] Session finished")
						return
					else:
