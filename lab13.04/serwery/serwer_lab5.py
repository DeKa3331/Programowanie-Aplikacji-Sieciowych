...
def run_server() -> None:
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
		...
		print(f"[serwer_lab5] SMTP image MIME server on {HOST}:{PORT}")
		print("[serwer_lab5] Waiting for one client...")

		connection, address = server_socket.accept()
		with connection:
			print(f"[serwer_lab5] Client connected: {address}")
			...
			while True:
				...
				if not chunk:
					print("[serwer_lab5] Client disconnected")
					break
				...
				while b"\n" in buffer:
					...
					if data_mode:
						if line == ".":
							...
							print(f"[serwer_lab5] MIME multipart present: {has_multipart}")
							print(f"[serwer_lab5] Base64 attachment marker present: {has_base64}")
							print(f"[serwer_lab5] Image content-type present: {has_image_type}")

							if has_multipart and has_base64 and has_image_type:
								send_line(connection, "250 MIME image message accepted")
							else:
								send_line(connection, "550 MIME image headers missing for zad5")
					...
					if upper_line.startswith("EHLO") or upper_line.startswith("HELO"):
						...
						print("[serwer_lab5] Session finished")
						return
					else:
...
def main() -> None:
	try:
		...
		print(f"[serwer_lab5] Socket error: {error}")
