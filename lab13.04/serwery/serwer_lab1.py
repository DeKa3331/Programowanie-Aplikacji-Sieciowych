...
def run_server() -> None:
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
		...
		print(f"[serwer_lab1] SMTP test server listening on {HOST}:{PORT}")
		print("[serwer_lab1] Waiting for one client...")

		connection, address = server_socket.accept()
		with connection:
			print(f"[serwer_lab1] Client connected: {address}")
			...
			while True:
				...
				if not chunk:
					print("[serwer_lab1] Client disconnected")
					break
				...
				while b"\n" in buffer:
					...
					if data_mode:
						if line == ".":
							print("[serwer_lab1] Received DATA body:")
							for msg_line in message_lines:
								print(f"[serwer_lab1]   {msg_line}")
							message_lines.clear()
							data_mode = False
